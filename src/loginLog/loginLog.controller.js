...
const { getAllLoginLog, getLoginLogById, createLoginLog, deleteLoginLogById, editLoginLogById, patchLoginLogById, getLoginStatsPerHour } = require('./loginLog.service');
...
router.get("/stats/hourly", async (req, res) => {
  try {
    const data = await getLoginStatsPerHour();
    res.json(data);
  } catch (err) {
    console.error(err);
    res.status(500).send("Gagal menghitung statistik login per jam");
  }
});
